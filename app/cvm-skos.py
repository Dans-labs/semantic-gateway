...
@app.route("/vocabs")
def vocabs():
    config = readconfig(request.args.to_dict())
...
def skosmos(keyword=None, vocab=None):
    keyword = "%s%s" % (keyword, '*')
    apiurl = "https://skosmos.labs.coronawhy.org/rest/v1/search?vocab=%s&query=%s" % (vocab, keyword)
    apiurl = "https://vocabulaires.irstea.fr/skosmos/rest/v1/search?vocab=thesaurus&query=%s" % keyword
    data = json.loads(requests.get(apiurl).text)
    result = data['results']
    dataset = {}
    alldata = []
